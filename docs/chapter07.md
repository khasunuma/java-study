...
型パラメータは、[10 章](chapter10.md)で紹介するジェネリクス (総称型) で用いるものと同じものです。クラスに対して、それが関連するクラスを明示するために使用されます。型パラメータの実際については [10 章](chapter10.md)もあわせて参照してください。
...
書式 (1) および (2) は通常のクラス・リテラル、書式 (3) および (4) は [10 章](chapter10.md) で取り上げる配列のクラス・リテラル、書式 (5) は特定のデータ型を表さない `void` のクラス・リテラルとなります。
