...
[10 章](chapter10.md)では「マルチスレッド」について触れます。マルチスレッドは複数の処理を同時並行で実行するための仕組みであり、Java では言語仕様レベルでサポートしている重要な機能です。マルチスレッドについて意識しなくても Java のプログラミング自体は可能ですが、後々大きな落とし穴にはまるケースが多く見受けられます。この章を通じて、感覚的な理解だけでも構いませんので、マルチスレッドを意識するようになってください。
