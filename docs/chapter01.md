...
>Java が `java.lang.Object` ようなクラスを用意しているのは、Smalltalk の影響によるものです。Java が最初に参考とした C++ には、すべてのクラスが暗黙的に継承するようなクラスを持たない仕様となっています。
...
>Java SE においては、現時点ではアノテーション (注釈) は限定的ですが、Java EE においては XML ファイルによる各種設定記述の代替として積極的に用いられています。Java EE のうち後発の API については「アノテーション (注釈) 地獄」と揶揄されるほど多用されています。
...
プリミティブ型のアイデアは C++ (および C) から拝借したものですが、Java の言語仕様に合わせてカスタマイズをしているため、類似はしているものの一致はしていません。
