...
>【対比】 論理リテラルは C、Python、JavaScript、Visual Basic にも存在します。他の言語では論理型と整数型は変換可能ですが (多くは 真 = 1、偽 = 0 となりますが、Visual Basic だけは真 = -1、偽 = 0)、Java ではできないことに注意してください。

#### 4.4.4.2. Null リテラル (NullLiteral)

いずれのインスタンスも参照しないことを示すリテラルを Null リテラルといい、キーワード `null` で表します。

>【対比】 C には言語レベルの Null はなく、マクロ定数 `NULL` が相当します (定義値はコンパイラに依存します)。Python は `None`、JavaScript は `null`、Visual Basic は `Nothing` がそれぞれ相当します。
