...
|`copyOf`|1.6指定された配列をコピーする (長さは必要に応じて切り詰めるかパディング)|
|`copyOfRange`|1.6指定された配列の指定された範囲を新しい配列にコピーする|
|`equals`|2 つの配列が互いに同等である場合に `true` を返す|
|`hashCode`|指定された配列の内容に基づくハッシュ・コードを返す|
|`toString`|指定された配列の文字列表現を返す|
|`deepEquals`|1.52つの配列が互いに等価な場合に `true` を返す (深層内容)|
|`deepHashCode`|1.5指定された配列のハッシュ・コードを返す (深層内容)|
|`deepToString`|1.5指定された配列の文字列表現を返す (深層内容)|
|`fill`|配列の各要素を設定する (同一の値を使用する)|
|`sort`|指定された配列をソートする|
|`setAll`|1.8配列のすべての要素を設定する (異なる値も設定可)|
|`parallelSort`|1.8指定された配列をソートする (マージソード・並列処理)|
|`parallelSetAll`|1.8配列のすべての要素を設定する (異なる値も設定可・並列処理)|
