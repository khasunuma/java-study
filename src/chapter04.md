...
#### 後置インクリメント式と後置デクリメント式

- 書式: `num++` (後置インクリメント式) または `num--` (後置デクリメント式)
- 数値 (整数型・浮動小数点型) の値を `+1` (後置インクリメント式) または `-1` (後置デクリメント式) します。
- 式を評価した後にその値を `+1` または `-1` します。データ型は変わりません。
  - 例えば、`index` の値が `3` の場合に式 `index++` の値は `3` と評価され、その後 `index` の値が `4` となります。
  - 同様に `index` の値が `3` の場合に式 `index--` の値は `3` と評価され、その後 `index` の値が `2` となります。
- 式の結合順序: `index++++` は `(index++)++`、`index----` は `(index--)--` として評価されます。
...
#### 前置インクリメント式と前置デクリメント式

- 書式: `++num` (前置インクリメント式) または `--num` (前置デクリメント式)
- 数値 (整数型・浮動小数点型) の値を `+1` (後置インクリメント式) または `-1` (後置デクリメント式) します。データ型は変わりません。
- 式の値を `+1` (前置インクリメント式) または `-1` (前置デクリメント式) した後に式を評価します。
  - 例えば、`index` の値が `3` の場合に式 `++index` の値が `4` となりその値で評価されます。
  - 同様に `index` の値が `3` の場合に式 `--index` の値は `2` となりその値で評価されます。
- 式の結合順序 (1): `++++index` は `++(++index)`、`----index` は `--(--index)` として評価されます。
- 式の結合順序 (2): `++index++` は `++(index++)`、`--index--` は `--(index--)` として評価されます (後置インクリメント式および後置デクリメント式の方が優先順位が高いため)。

>【対比】 前置インクリメント式および前置デクリメント式は C および JavaScript には存在しますが、Python および Visual Basic には存在しません。

#### 符号

- 書式: `+num` または `-num` (符号省略時は `+num` と見なされる)
- 数値 (整数型・浮動小数点型) の符号を維持 (`+num`) または反転 (`-num`) します。データ型は変わりません。
- 式の結合順序 (1): `+ +num`、`- -num` は `+(+num)`、`-(-num)` として評価され、最終的には `(num)`、`(num)` と評価されます。
- 式の結合順序 (2): `++num`、`--num` と表記するとそれぞれ前置インクリメント式、前置デクリメント式となり、符号の二重適用にはなりません (優先順位の高い前置インクリメント式、前置デクリメント式が優先されるため)。
...
>【対比】 関係演算式は C、Python、JavaScript、Visual Basic に存在し、記法・用法も同じです。
...
|分類|代入演算子|基本形式|結合順序|
|----|----------|--------|--------|
|単純代入演算子|`lhs = expr`|`lhs = expr`|`lhs = (expr)`|
|復号代入演算子|`lhs *= expr`|`lhs = lhs * expr`|`lhs = (lhs * expr)`|
