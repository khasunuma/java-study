...
キャッチすべき例外が複数存在する場合は、以下の例のように catch (例外) { ... } が複数 (0 個以上) 存在します。また、finally { ... } の処理が必要ない場合は、finally 自体を省略することもできます。finally はファイルやネットワークなど、終了処理を必ず行わなければならない処理のために用意されています。実は try-catch 文で必須なのは try 節だけで、catch 節と finally 節の両方を省略することもできますが、後述の try-with-resources 文を除き何らかの処理が存在しているはずです。
