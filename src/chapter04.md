...
>例外処理は C および Visual Basic には存在しません。Visual Basic には On-Error-GoTo 構文によるエラーハンドリング機構を有していますが、例外処理はより高度な機能です。C++、JavaScript にもそれぞれ例外処理機構は備わっていますが、いずれも Java とは異なっています。
